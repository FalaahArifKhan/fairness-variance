...
def create_stress_testing_sets_using_cols_importance(original_X_test, original_y_test, error_injector,
                                                     injector_config_lst, fitted_column_transformer):
    # Create test sets for model stress testing
    extra_test_sets_lst = []
    for column_name in injector_config_lst:
        X_test = original_X_test.copy(deep=True)
        error_injector.set_columns_to_transform([column_name])
